...
// 修改部分代码,让开发启动时,自动生成swagger文档,不必单独命令行启动,示例
//if beego.BConfig.RunMode == "dev" {
//
//		currentpath, _ := os.Getwd()
//		tools.ParsePackagesFromDir(currentpath)
//		tools.GenerateDocs(currentpath)
//
...
func parseObject(d *ast.Object, k string, m *swagger.Schema, realTypes *[]string, astPkgs []*ast.Package, packageName string) {
	...
	case *ast.ArrayType:
		...
		if isBasicType(fmt.Sprint(t.Elt)) {
			//beeLogger.Log.Error(fmt.Sprint(t.Elt))
